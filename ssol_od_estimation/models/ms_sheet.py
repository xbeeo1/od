...
class MsSheet(models.Model):
    _name = "ms.sheet"
    _description = "Ms Sheet"
    ...
    ms_sheet_line = fields.One2many("ms.sheet.line", "order_id", string="")
    ...
    @api.depends('total_amount', 'ms_sheet_line', 'ms_sheet_line.price_total')
    def _compute_total_amount(self):
        for rec in self:
            total_amount = 0
            for line in rec.ms_sheet_line:
    ...
    def unlink(self):
        for rec in self:
            ...
            if not rec.env.user.has_group('ssol_od_estimation.group_order_estimation_delete_access'):
                ...
                return super(MsSheet, self).unlink()
...
class MsChannelLine(models.Model):
    ...
    order_id = fields.Many2one('ms.sheet', string='Estimation Order Reference', required=True, ondelete='cascade')
    ...
    ms_sheet_size = fields.Selection(string="Size", selection=[('26G   (0.46 mm)', '26G   (0.46 mm)'),
                                                               ('24G   (0.56 mm)', '24G   (0.56 mm)'),
                                                               ('22G   (0.71 mm)', '22G   (0.71 mm)'),
                                                               ('20G   (0.91 mm)', '20G   (0.91 mm)'),
                                                               ('18G   (1.22 mm)', '18G   (1.22 mm)'),
                                                               ('16G   (1.63 mm)', '16G   (1.63 mm)'),
                                                               ('14G   (2.03 mm)', '14G   (2.03 mm)'),
                                                               ('12G   (2.64 mm)', '12G   (2.64 mm)'),
                                                               ], default='26G   (0.46 mm)')
    ...
    weight_fit = fields.Float(string="Weight/Piece (kg)", digits=(16, 2), compute='_compute_total_weight_fit')
    total_weight = fields.Float(string="Total Weight (kg)", digits=(16, 2), compute='_compute_total_weight')

    @api.depends('ms_sheet_size', 'length', 'width', 'total_weight', 'product_id', 'weight_per_sqft')
    def _compute_weight_per_sqft(self):
        for line in self:
            if line.ms_sheet_size == '26G   (0.46 mm)':
                line.weight_per_sqft = 0.36281

            elif line.ms_sheet_size == '24G   (0.56 mm)':
                line.weight_per_sqft = 0.45938

            elif line.ms_sheet_size == '22G   (0.71 mm)':
                line.weight_per_sqft = 0.57813

            elif line.ms_sheet_size == '20G   (0.91 mm)':
                line.weight_per_sqft = 0.71875

            elif line.ms_sheet_size == '18G   (1.22 mm)':
                line.weight_per_sqft = 0.90625

            elif line.ms_sheet_size == '16G   (1.63 mm)':
                line.weight_per_sqft = 1.1875

            elif line.ms_sheet_size == '14G   (2.03 mm)':
                line.weight_per_sqft = 1.4375

            elif line.ms_sheet_size == '12G   (2.64 mm)':
                line.weight_per_sqft = 1.8125
    ...
    @api.depends('total_weight', 'weight_fit', 'weight_fit', 'ms_sheet_size', 'product_id', 'length', 'width', 'weight_fit', 'product_uom_qty', 'weight_per_sqft')
    ...
    @api.depends('total_weight', 'weight_fit', 'weight_fit', 'ms_sheet_size', 'product_id', 'length', 'width',
    ...
    @api.depends('order_id.ms_sheet_line', 'sr_no', 'sr_no_char')
    def _compute_sr_no(self):
        for order in self.mapped('order_id'):
            value_sr = 1
            for line in order.ms_sheet_line:
                value_sr += 1
