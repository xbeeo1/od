...
class CopperSheet(models.Model):
    _name = "copper.sheet"
    _description = "Copper Sheet"
    ...
    copper_sheet_line = fields.One2many("copper.sheet.line", "order_id", string="")
    ...
    @api.depends('total_amount', 'copper_sheet_line', 'copper_sheet_line.price_total')
    def _compute_total_amount(self):
        for rec in self:
            total_amount = 0
            for line in rec.copper_sheet_line:
    ...
    def unlink(self):
        for rec in self:
            ...
            if not rec.env.user.has_group('ssol_od_estimation.group_order_estimation_delete_access'):
                ...
                return super(CopperSheet, self).unlink()



class CopperSheetLine(models.Model):
    _name = 'copper.sheet.line'


    order_id = fields.Many2one('copper.sheet', string='Order Reference', required=True, ondelete='cascade')
    ...
    gage_size = fields.Selection(string="SWG",
                                 selection=[('34', '34'),
                                            ('36', '36'), ('37', '37'),
                                            ('38', '38'), ('40', '40'),
                                            ], default='34')  # fetch data from copper tubes

    sheet_inches = fields.Float(string="Inch",digits=(16, 4), compute='_compute_inches_sheet_mm_weight')
    sheet_mm = fields.Float(string="MM", digits=(16, 3), compute='_compute_inches_sheet_mm_weight')
    copper_weight = fields.Float(string="KG/Sq.ft", digits=(16, 2), compute='_compute_inches_sheet_mm_weight')
    ...
    @api.depends('gage_size', 'sheet_inches', 'sheet_mm', 'copper_weight', 'product_id')
    def _compute_inches_sheet_mm_weight(self):
        for line in self:
            if line.gage_size == '34':
                line.sheet_inches = 0.0092
                line.sheet_mm = 0.234
                line.copper_weight = 0.194

            elif line.gage_size == '36':
                line.sheet_inches = 0.0076
                line.sheet_mm = 0.193
                line.copper_weight = 0.157

            elif line.gage_size == '37':
                line.sheet_inches = 0.0055
                line.sheet_mm = 0.15
                line.copper_weight = 0.123

            elif line.gage_size == '38':
                line.sheet_inches = 0.0060
                line.sheet_mm = 0.152
                line.copper_weight = 0.125

            elif line.gage_size == '40':
                line.sheet_inches = 0.0048
                line.sheet_mm = 0.122
                line.copper_weight = 0.167
    ...
    @api.depends('total_weight', 'copper_weight', 'gage_size', 'length','width', 'product_uom_qty', 'product_id')
    def _compute_total_weight(self):
        for line in self:
            if line.length and line.width and line.product_uom_qty:
                total_sqft = line.length * line.width
                mm_value = total_sqft/92903
                sub_weight = mm_value * line.copper_weight
    ...
    @api.depends('order_id.copper_sheet_line', 'sr_no', 'sr_no_char')
    def _compute_sr_no(self):
        for order in self.mapped('order_id'):
            value_sr = 1
            for line in order.copper_sheet_line:
                value_sr += 1
